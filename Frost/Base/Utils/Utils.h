...
#define log(str) OutputDebugString(str)
    ...
    bool HookFunction(void* pTarget, T pDetour, void* pOriginal, const char* SignatureIdName) {
        ...
        if (!initialized)
        {
            ...
            {
                log("[frost] Failed to initialize MinHook for the client.\n");
                return false;
            }
        }
        ...
        {
            log(combine("[frost] Failed to create hook for ", SignatureIdName, "\n").c_str());
            return false;
        }

        if (MH_EnableHook(pTarget) != MH_OK)
        {
            log(combine("[frost] Failed to enable hook for ", SignatureIdName, "\n").c_str());
            return false;
        }

        log(combine("[frost] Successfully applied hook for ", SignatureIdName, "\n").c_str());
